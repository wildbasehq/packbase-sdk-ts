...
Render metadata specially for notification types your application recognizes. For unknown types, fall back to recursively displaying JSON primitives, arrays, and object keys—or use formatted `JSON.stringify` output—so new server metadata never produces a blank notification.
