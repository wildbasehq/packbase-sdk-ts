...
`Pack` describes a Packbase community that the API returns. Optional fields end in `?`. These fields can be absent because of the endpoint or authentication state. Some fields can also be `null`. `null` means that Packbase does not have a stored value.
